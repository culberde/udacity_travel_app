...
function geoNames(name, dateText) {
  request(`http://api.geonames.org/search?q=${name}&maxRows=1&username=culberde`, {json: true}, (err, res, body) => {
    if (err) {
...
function darkSky(lat, lon, dateText) {
  ...
  if (type == 'current') {
    request(`https://api.darksky.net/forecast/${process.env.DARKSKY_APIKEY}/${lat},${lon}`, {json: true}, (err, res, body) => {
      if (err) { return console.log(err); }
      return createWeeklyFcst(body.daily.data);
    })
  }
  else {
    request(`https://api.darksky.net/forecast/${process.env.DARKSKY_APIKEY}/${lat},${lon},${date}`, {json: true}, (err, res, body) => {
      if (err) { return console.log(err); }
